...
@Repository
public class YwPGrhydjDao extends BaseDaoImpl {
    private static int pageSize = 5000;

    public List<YwPGrhydjEntity> getData(int currentPage, boolean firstFtpUpload, DpFtpUploadLogEntity recentlyData) {
        SimplePage simplePage = new SimplePage(currentPage, pageSize);

        // 如果有ftp上传记录，则不是首次上传，那么进行增量上传
        if (firstFtpUpload) { // 首次全量上传数据
            Pageable<YwPGrhydjEntity> page = this.findWithPage(YwPGrhydjEntity.class, simplePage);
            return page.getList();
        } else {
            ...
            Pageable<YwPGrhydjEntity> page = this.findWithPage(YwPGrhydjEntity.class, simplePage, queryConditions);
            return page.getList();
        }
    }
    ...
    public int getPageCount(boolean firstFtpUpload, DpFtpUploadLogEntity recentlyData) {
        long totalRecord;
        if (firstFtpUpload) { // 首次全量上传数据
            totalRecord = this.count(YwPGrhydjEntity.class, (QueryCondition) null);
        } else {
            Calendar cal = Calendar.getInstance();
            if (recentlyData != null && recentlyData.getCreateTime() != null) {
                cal.setTime(recentlyData.getCreateTime());
            ...
            totalRecord = this.count(YwPGrhydjEntity.class, queryConditions);
        }
