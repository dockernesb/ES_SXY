...
import com.udatech.dataInterchange.dao.YwPGrhydjDao;
...
import com.udatech.dataInterchange.vo.YwPGrhydjDatas;
...
@Service
public class YwPGrhydjService {
    private Logger logger = Logger.getLogger(YwPGrhydjService.class);
    private static String tableCode = "YW_P_GRHYDJ";
    private static String talbeNo = "05";

    @Autowired
    private YwPGrhydjDao YwPGrhydjDao;
    @Autowired
    ...
    public void genXml() {
        ...
        if (recentlyData == null) {
            ...
            int pageCount = YwPGrhydjDao.getPageCount(true, recentlyData);
            if (pageCount == 0) {
                // 本日无数据，记录上传日志信息
            ...
            if (recentlyData.getCurrentPage() == recentlyData.getAllPage()) {
                ...
                int pageCount = YwPGrhydjDao.getPageCount(false, recentlyData);
                if (pageCount == 0) {
                    // 本日无数据，记录上传日志信息
                    ftpDao.addNoDataLog(tableCode);
                    return;
    ...
    private void genXml(int currentPage, int allPage, boolean firstFtpUpload, DpFtpUploadLogEntity recentlyData) {
        for (int i = currentPage; i <= allPage; i++) {
            List<YwPGrhydjEntity> list = YwPGrhydjDao.getData(i, firstFtpUpload, recentlyData);

            String filePath = genXmlService.getXmlFilePath(talbeNo);

            YwPGrhydjDatas YwLHeimingdanDatas = new YwPGrhydjDatas();
            YwLHeimingdanDatas.setData(list);
            JaxbUtil requestBinder = new JaxbUtil(YwPGrhydjDatas.class, JaxbUtil.CollectionWrapper.class);
            try {
