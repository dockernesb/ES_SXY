...
import com.udatech.dataInterchange.dao.YwLJgslbgdjDao;
...
import com.udatech.dataInterchange.vo.YwLJgslbgdjDatas;
...
@Service
public class YwLJgslbgdjService {
    private Logger logger = Logger.getLogger(YwLJgslbgdjService.class);
    private static String tableCode = "YW_L_JGSLBGDJ";
    private static String talbeNo = "06";

    @Autowired
    private YwLJgslbgdjDao ywLJgslbgdjDao;
    @Autowired
    ...
    public void genXml() {
        ...
        if (recentlyData == null) { // 无xml生成记录，表示之前从来没有生成过，本次是首次生成
            int pageCount = ywLJgslbgdjDao.getPageCount(true, recentlyData);
            if (pageCount == 0) {
                // 本日无数据，记录上传日志信息
            ...
            if (recentlyData.getCurrentPage() == recentlyData.getAllPage()) {
                ...
                int pageCount = ywLJgslbgdjDao.getPageCount(false, recentlyData);
                if (pageCount == 0) {
                    // 本日无数据，记录上传日志信息
                    ftpDao.addNoDataLog(tableCode);
                    return;
    ...
    private void genXml(int currentPage, int allPage, boolean firstFtpUpload, DpFtpUploadLogEntity recentlyData) {
        for (int i = currentPage; i <= allPage; i++) {
            List<YwLJgslbgdjEntity> list = ywLJgslbgdjDao.getData(i, firstFtpUpload, recentlyData);

            String filePath = genXmlService.getXmlFilePath(talbeNo);

            YwLJgslbgdjDatas YwLHeimingdanDatas = new YwLJgslbgdjDatas();
            YwLHeimingdanDatas.setData(list);
            JaxbUtil requestBinder = new JaxbUtil(YwLJgslbgdjDatas.class, JaxbUtil.CollectionWrapper.class);
            try {
