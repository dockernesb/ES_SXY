...
import com.udatech.dataInterchange.dao.YwLSgsxzxkDao;
...
import com.udatech.dataInterchange.vo.YwLSgsxzxkDatas;
...
@Service
public class YwLSgsxzxkService {
    private Logger logger = Logger.getLogger(YwLSgsxzxkService.class);
    private static String tableCode = "YW_L_SGSXZXK";
    private static String talbeNo = "02";

    @Autowired
    private YwLSgsxzxkDao ywLSgsxzxkDao;
    @Autowired
    ...
    public void genXml() {
        ...
        if (recentlyData == null) {
            ...
            int pageCount = ywLSgsxzxkDao.getPageCount(true, recentlyData);
            if (pageCount == 0) {
                // 本日无数据，记录上传日志信息
            ...
            if (recentlyData.getCurrentPage() == recentlyData.getAllPage()) {
                ...
                int pageCount = ywLSgsxzxkDao.getPageCount(false, recentlyData);
                if (pageCount == 0) {
                    // 本日无数据，记录上传日志信息
                    ftpDao.addNoDataLog(tableCode);
                    return;
    ...
    private void genXml(int currentPage, int allPage, boolean firstFtpUpload, DpFtpUploadLogEntity recentlyData) {
        for (int i = currentPage; i <= allPage; i++) {
            List<YwLSgsxzxkEntity> list = ywLSgsxzxkDao.getData(i, firstFtpUpload, recentlyData);

            String filePath = genXmlService.getXmlFilePath(talbeNo);

            YwLSgsxzxkDatas YwLHeimingdanDatas = new YwLSgsxzxkDatas();
            YwLHeimingdanDatas.setData(list);
            JaxbUtil requestBinder = new JaxbUtil(YwLSgsxzxkDatas.class, JaxbUtil.CollectionWrapper.class);
            try {
