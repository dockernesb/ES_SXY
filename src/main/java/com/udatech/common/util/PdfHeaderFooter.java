...
public class PdfHeaderFooter extends PdfPageEventHelper {
     ...
     * @param header 页眉字符串
     * @param presentFontSize 页眉页脚字体大小
     ...
     * @param bf 基础字体对象
     */
    public PdfHeaderFooter(String header, int presentFontSize, Rectangle pageSize, BaseFont bf) {
        this.header = header;
     ...
     * @param bf 基础字体对象
     */
    public PdfHeaderFooter(BaseFont bf) {
        this.bf = bf;
    }
     ...
     * @param presentFontSize 页眉页脚字体大小
     */
    public PdfHeaderFooter(String header, int presentFontSize) {
        this.header = header;
        this.presentFontSize = presentFontSize;
    }
     ...
     * @param header 页眉字符串
     */
    public PdfHeaderFooter(String header) {
        this.header = header;
    }

    /**
     * @param header 页眉字符串
     * @param bfChinese 基础字体对象
     */
    public PdfHeaderFooter(String header, BaseFont bfChinese) {
        this.header = header;
        this.bf = bfChinese;
    }
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        if (pageNumber > 1) {// 跳过第一页的封面，不显示页眉页脚
            try {
                if (bf == null) {
                    bf = BaseFont.createFont("C:/windows/fonts/SIMSUN.TTC,0", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
                }
                if (fontDetail == null) {
            ...
            // 1.写入页眉
            ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_LEFT, new Phrase(header, fontDetail), document.left(),
                            document.top() + 20, 0);

            // 2.写入前半部分的 第 X页/共
            String foot1 = "第 " + (pageNumber - 1) + " 页  共";
    ...
    public void onCloseDocument(PdfWriter writer, Document document) {
        ...
        String foot2 = " " + (writer.getPageNumber() - 2) + " 页";
