        ...
        $('.select2-hidden-accessible.form-search').next('span').css('margin-bottom', '5px');
		  ...
		  var end = {
			...
			function addTable(){
				...
				$.post(ctx + "/dpTheme/getMsgColumns.action", {
					'msgType': $("#messageType").val()
				}, function(data) {	
					if (table) {
						table = null;
					...
					arr.push({
					 ...
					 $("#tableId").append('<th>上报省状态</th><th>操作</th>');
					 arr.push({
					 ...
					 arr.push({
						 ...
						 "render" : function(data, type, full){
							 var str = '';
							   if(data.PROVINCE_STATUS ==0 ||data.PROVINCE_STATUS ==2){
					 ...
					 table = $('#dataReportGrid').DataTable({
				        ajax: {
				            url: ctx + "/dpTheme/themePage/tableList.action",
				            type: "post",
				            data: { 
				            	msgType : $("#messageType").val(),
		    ...
		    function commitData(id){
						...
						layer.confirm("确定提交吗？", {
							...
							layer.close(index);
							loading();
							$.post(CONTEXT_PATH + "/dpTheme/commitDataList.action",{
								'id': id,
								'msgType': $("#messageType").val()
								},
									function(data){
								loadClose();
								if (!data.result) {
									$.alert('报送失败，失败原因请在数据报送记录中查看详情!', 2);
								} else {
									layer.close(index);
									$.alert('提交成功!', 1);
									queryData();
		    ...
		    $("#commitProvinceData").click(function(){
		    	commitProvince();
		    });
		    
		    function commitProvince(){
		    	 var rows = new Array();
				...
				layer.confirm("确定提交这些数据吗？", {
					icon : 3,
				}, function(index) {
					layer.close(index);
					loading();
					$.post(CONTEXT_PATH +"/dpTheme/batchCommitDataList.action",{
