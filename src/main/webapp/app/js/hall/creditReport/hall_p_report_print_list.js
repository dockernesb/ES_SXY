var holPPrint = (function() {
	...
	var table = $('#dataTable').DataTable({
		...
		columns : [ {
			...
			"data" : "xybgbh",
			"render" : function(data, type, full) {
				var opts = '<a href="javascript:;" onclick="holPPrint.toReportViewPrint(\'' + data + '\', \'' + full.id + '\', this, \'' + full.cxrxm + '\')">' + data + '</a>';
				return opts;
			}
		}, {
			"data" : "cxrxm"
	...
	function refreshTable(bjbh, cxrxm, cxrsfzh, xybgbh, beginDate, endDate, deptId) {
		if (table) {
	...
	$('.printCityBtn').on('click', function() {
		...
		if (selectedRows.length > 0) {
			...
			toReportViewPrint(row.xybgbh, row.id, null, row.cxrxm);
		} else {
			$.alert('请选择要操作的记录！');
		}
	});
	...
	function conditionSearch() {
		...
		refreshTable(bjbh, cxrxm, cxrsfzh, xybgbh, beginDate, endDate, deptId);
	}

	function conditionReset() {
		resetSearchConditions('#bjbh,#cxrxm,#cxrsfzh,#beginDate,#endDate,#xybgbh,#deptId');
		resetDate(start, end);
	}
