(function() {
	...
	function saveObjectionAudit(status) {
		$.post(ctx + "/govObjection/saveObjectionAudit.action", {
			"id" : detailId,
			"status" : status,
			...
			if (result.result) {
				$.alert(result.message, 1, function() {
					...
					var activeIndex = recordDtActiveIndex(gol.table);
					gol.table.ajax.reload(function(){
			        	callbackDtRowActive(gol.table, activeIndex);
			        }, false);// 刷新列表还保留分页信息
				});
			} else {
