(function() {
		...
		$.each(theme.children, function(j, themeTwo) {// 二级资源
			...
			var table = $(tableId).DataTable({// 创建一个Datatable
				...
				createdRow : function(row, data, dataIndex) {
					if (data.STATUS == '2') { // 已修复的数据，特殊显示
						$(row).css({
							'background-color' : '#e35b5a',
							'color' : '#fff'
						});
					}
					if (data.isObjection == "true" || data.isObjection) { // 有争议的数据，特殊显示
						$(row).css({
				...
				drawCallback : function(settings) {
					...
					checkAll.on('ifChecked ifUnchecked', function(event) {
						if (event.type == 'ifChecked') {
							...
							// 添加字段选中记录
							$.each(table.columns({page:'current'}).data(), function(rowIndex, rowData) {
								// 将数据设置为选中状态
	...
	function generateReport() {
		var url = ctx + "/reportQuery/generatePReportView.action";
		var body = {
			params : params,
			...
			if (data.result) {
				$.alert('审核成功！', 1, function() {
					...
					if (isAcrobatPluginInstall()) {
						var url = ctx + '/reportQuery/viewReport.action?xybgbh=' + data.message + "&jgqc=" + $("#jgqc").val();
	...
	function saveReportAudit(status) {
		$.post(ctx + "/creditPReport/reportApplyAudit.action", {
			"id" : businessId,
			"status" : status,
			"zxshyj" : zxshyj
		}, function(result) {
	...
	$(".backBtn").click(function() {
		...
		callbackSelectNull(selectArr);
		var activeIndex = recordDtActiveIndex(centerPReport.table);
		centerPReport.table.ajax.reload(function(){
        	callbackDtRowActive(centerPReport.table, activeIndex);
        }, false);// 刷新列表还保留分页信息
		resetIEPlaceholder();
	});
})();