var promiseAddEnter = (function() {
	...
	var table = $('#enterGrid').DataTable({
		...
		"pageLength" : 10,
		ajax : {
			url : CONTEXT_PATH + '/sszj_promise/queryList.action?cnlb=' + cnlb + '&deptId=' + deptId, // 后台地址
			type : 'post'
		},
	...
	function toViewEnterDetail(enterId) {
		var url = CONTEXT_PATH + '/sszj_promise/toViewEnterDetail.action?enterId=' + enterId + '&supvId=' + supvId;
	...
	function manualAddSub(index) {
		...
		$.post(CONTEXT_PATH + '/sszj_promise/manualAdd.action', {
			qymc : qymc,
			tyshxydm : shxydm,
			zzjgdm : zzjgdm,
			gszch : gszch,
	...
	function batchAdd(filePathStr, fileNameStr) {
		loading();
		$.post(CONTEXT_PATH + '/sszj_promise/batchAdd.action', {
			cnlb : cnlb,
			deptId : deptId,
	...
	function templateDownload() {
		loading();
		window.location.href = CONTEXT_PATH + "/sszj_promise/templateDownload.action";
		loadClose();
	}
	...
	function toPromiseQyHandle(id, _this) {
		addDtSelectedStatus(_this);
		var url = ctx + "/sszj_promise/toPromiseQyHandle.action?id=" + id;
	...
	function toPromiseQyView(id, _this) {
		addDtSelectedStatus(_this);
		var url = ctx + "/sszj_promise/toPromiseQyView.action?id=" + id + "&from=2";
        $("div#child_childDiv").html("");
        $("div#child_childDiv").load(url);
        $("div#child_childDiv").prependTo("#childTopBox");
        $("div#child_fatherDiv").hide();
