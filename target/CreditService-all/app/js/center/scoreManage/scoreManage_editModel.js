...
var editModel = (function() {
	...
	var saveEditModelForm = $('#saveEditModelForm').validateForm(rules);
	...
	var table = $('#appGrid').DataTable({
		...
		columns : [ {
			...
			render : function(data) {
				var str = '<button type="button" class="btn btn-xs yellow" style="margin-top: 3px;margin-right: 10px;float: right;" onclick="editModel.addQyzz(\'' + data + '\');"> 选择</button>';
				return str;
			}
		} ]
	});
	...
	function addQyzz(zzmc) {
		...
		if (flg == 1) {
			...
			str += "	</td>";
			str += "	<td align='center' nowrap>";
			str += "        <button type='button' class='btn btn-xs red' style='margin-top: 3px;margin-right: 10px;float: right;' onclick='editModel.delRow(this);'> 删除</button>";
			str += "	</td>";
			str += "</tr>";
			var $str = $(str);
			$("#qyzz_qj tbody").append($str); // 在table的最后一行前面添加一行
			$("input").blur(function() {
				saveEditModelForm.element($(this));
			});
		}
	}
	...
	// 校验tab_1
	function validTab1() {
		var res_mc = $("#saveEditModelForm").validate().element($("#cjpfmx_mxmc"));
		var res_ms = $("#saveEditModelForm").validate().element($("#cjpfmx_mxms"));
		if (res_mc && res_ms) {
			return true;
		} else {
			return false;
	...
	function validTab2() {
		...
		$('#tab_2 input[type=greaterThanZero]').each(function() {
			var tab2 = $("#saveEditModelForm").validate().element(this);
			if (!tab2) {
				res = false;
			}
		});
	...
	function validTab3() {
		...
		$('#tab_3 input[type=greaterThanZero]').each(function() {
			var tab = $("#saveEditModelForm").validate().element(this);
			if (!tab) {
				res = false;
			}
		});
	...
	function saveEditModel() {
		...
		loading();
		$('#saveEditModelForm').ajaxSubmit({
			url : CONTEXT_PATH + '/center/scoreManage/saveEditModel.action',
			dataType : "json",
			success : function(data) {
				loadClose();
				if (data.result) {
					$.alert("更新成功", 1, goBackList());
				} else {
					var msg = "";
					if (data.message) {
						msg = "(" + (data.message || "") + ")";
	...
	function addRowSForOld() {
		...
		$.each(qyzzMapList, function(i, value) {
			...
			str += "	<input name='qyzz_zzmc' type='hidden' class='form-control' value='" + value.ZLMC + "'/>";
			str += "	</td>";
			str += "	<td align='center' nowrap >";
			str += '        <div class="input-icon right"><i class="fa"></i>';
			str += '		<input name="qyzz_fs" value="' + value.SCORE + '" type="greaterThanZero" maxlength="3" required class="form-control"/>';
			str += '        </div>';
			str += "	</td>";
			str += "	<td align='center' nowrap>";
			str += "        <button class='btn btn-xs red' style='margin-top: 3px;margin-right: 10px;float: right;' onclick='editModel.delRow(this);'> 删除</button>";
			str += "	</td>";
			str += "</tr>";
			var $str = $(str);
			$("#qyzz_qj tbody").append($str); // 在table的最后一行前面添加一行
	...
	function addTrForOld(pfzbName, pfzbMin, pfzbMax, pfzbScore, flg, tbodyName) {
		if (pfzbMax != null) {
			...
			var newRow = $("#trForAdd tr:eq(" + flg + ")").clone(true); // 获取tr
	...
	function goBackList() {
        ...
        $("div#fatherDiv").show();
        $("div#childDiv").hide();
        var activeIndex = recordDtActiveIndex(smml.table);
