var blackList = (function () {
    ...
    var table = $('#blackGrid').DataTable(// 创建一个Datatable
        {
            ajax: {
                url: CONTEXT_PATH + "/center/publicity/getBlackList.action",
                type: 'post'
            },
            ...
            columns: [{
                ...
                "render": function (data, type, row) {
                    var str = "";
                    if (!isNull(data)) {
                        str = "<a href='javascript:void(0);' onclick='blackList.showDetail(\"" + row.ID + "\")'>" + data + "</a>";
                    }
                    return str;
                }
            }, {
                ...
                "data": null,
                "render": function (data, type, row) {
                    var opts = "<a href='javascript:void(0);' class='opbtn btn btn-xs green-meadow' onclick='blackList.showDetail(\"" + row.ID + "\")'>查看</a>";
                    if (row.STATUS == 1) {
                        opts += "<a href='javascript:void(0);' class='opbtn btn btn-xs green-meadow' onclick='blackList.changeBlackStatus(\"" + row.ID + "\", \"2\")'>开始公示</a>";
                    } else if (row.STATUS == 2) {
                        opts += "<a href='javascript:void(0);' class='opbtn btn btn-xs green-meadow' onclick='blackList.changeBlackStatus(\"" + row.ID + "\", \"3\")'>停止公示</a>";
                    }
                    return opts;
                }
            }],
            ...
            drawCallback: function (settings) {
                $('#blackGrid .checkall').iCheck('uncheck');
                $('#blackGrid .checkall, #blackGrid tbody .icheck').iCheck({
                    labelHover: false,
                    cursor: true,
                    checkboxClass: 'icheckbox_square-blue',
                    radioClass: 'iradio_square-blue',
                ...
                // 列表复选框选中取消事件
                var checkAll = $('#blackGrid .checkall');
                var checkboxes = $('#blackGrid tbody .icheck');
                checkAll.on('ifChecked ifUnchecked', function (event) {
                    if (event.type == 'ifChecked') {
                        checkboxes.iCheck('check');
                        $('#blackGrid tbody tr').addClass('active');
                    } else {
                        checkboxes.iCheck('uncheck');
                        $('#blackGrid tbody tr').removeClass('active');
                    }
                });
                ...
                // 添加行选中点击事件
                $('#blackGrid tbody tr').on('click', function () {
    ...
    function showDetail(id) {
        loading();
        $.getJSON(CONTEXT_PATH + "/center/publicity/getDetailBlackById.action", {
            id: id
        }, function (data) {
            loadClose();
            var detail = data;
    ...
    // 开始|停止公示
    function changeBlackStatus(id, status) {
        var tips = status == 2 ? "确认开始公示吗？" : "确认停止公示吗？";
        layer.confirm(tips, {
            icon: 3
        }, function (index) {
            loading();
            $.post(CONTEXT_PATH + "/center/publicity/changeBlackStatus.action", {
                "id": id,
                "status": status
            }, function (data) {
                loadClose();
    ...
    function exportResult() {
        ...
        if (rowIds.length !== 0) {
            layer.confirm('确定按照所有勾选数据进行导出吗？', {}, function () {
                ...
                document.location = CONTEXT_PATH + "/center/publicity/exportAllBlackList.action?rowIds=" + rowIds;
            });
        } else {
            layer.confirm('确定要导出全部数据记录吗？', {}, function () {
                ...
                var url = CONTEXT_PATH + "/center/publicity/exportAllBlackList.action";
                var _form = $("<form></form>", {
                    'id': 'importExcel',
                    'method': 'post',
                    'action': url,
    ...
    return {
        ...
        "changeBlackStatus": changeBlackStatus,
