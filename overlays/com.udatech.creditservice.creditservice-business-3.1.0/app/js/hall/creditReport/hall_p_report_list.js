var holPRead = (function() {
	...
	var table = $('#dataTable').DataTable({
		...
		columns : [ {
			...
			"data" : "xybgbh",
			"render" : function(data, type, full) {
				var opts = '<a href="javascript:;" onclick="holPRead.toReportViewPrint(\'' + data + '\', \'' + full.id + '\', this)">' + data + '</a>';
				return opts;
			}
		}, {
			"data" : "cxrxm"
	...
	function conditionReset() {
		if (audit == '1') {
			resetSearchConditions('#bjbh,#cxrxm,#cxrsfzh,#beginDate,#endDate,#xybgbh,#status,#isHasBasic');
		} else {
			resetSearchConditions('#bjbh,#cxrxm,#cxrsfzh,#beginDate,#endDate,#xybgbh,#isHasBasic');
		}
		resetDate(start, end);
	}
