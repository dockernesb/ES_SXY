(function() {
	...
	var handleTitle = function(tab, navigation, index) {
        ...
        if (current == 4) {
        	...
        	var yyxxsqbLen = $("input[name='yyxxsqbPath']").length;
        	if (yyxxsqbLen <= 0) {
        		$.alert("请上传异议信息申请表！");
                return false;
        	}
			...
			yes : function(index) {
				...
				if ($list.length > 0) {
					...
					$.each($list, function(i, obj) {
						...
						html += '<input type="hidden" class="objectionInfo" name="deptCode" value="' + bmbm + '" />';
						html += '<input type="hidden" class="objectionInfo" name="deptName" value="' + bmmc + '" />';
	...
	$(".button-submit").click(function() {
		...
		$("#submit_form").ajaxSubmit({
			url : ctx + "/hallObjection/addObjection.action",
				...
				if (result.result) {
					layer.confirm(result.message, {
						...
						cancel: function(index){ 
							var url = ctx + "/hallObjection/toObjectionApply.action";
		            	...
		            	layer.close(index);
		            	var url = ctx + "/hallObjection/printObjection.action?id=" + result.id;
		                window.open(url, "异议申诉反馈单");
		                url = ctx + "/hallObjection/toObjectionApply.action";
		                $("div#mainContent").load(url);
		            });
				} else {
