var creditTemplete = function() {
	...
	function addTemplate() {
		var addUrl = ctx + '/creditTemplate/toConfigureTemplate.action?opType=add&templateId=&useType=0';
	...
	function editTemplate() {
		if (!isNull($('.nav-tabs >.active >a').attr('href'))) {
			...
			var addUrl = ctx + '/creditTemplate/toConfigureTemplate.action?opType=update&templateId=' + templateId + '&useType=0';
	...
	function previewReport(em) {
		if (!isNull($(em)) && !isNull($(em).attr('href'))) {
			...
			// 加载模板报告预览
			var previewUrl = ctx + '/creditTemplate/previewReport.action?templateId=' + templateId;
	...
	function watermarkManage() {
		var url = ctx + '/creditTemplate/watermark.action?useType=0';
		$('#templateRow').hide();
		$('#templateContent').html("");
		$('#templateContent').show();
