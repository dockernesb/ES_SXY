...
var cCreditCheckList = (function() {
	$.getJSON(ctx + "/system/department/getDeptList.action?isIncludedAll=true", function(result) {
		...
		$('#xqbm,#examineList_status').val(null).trigger("change");
		resizeSelect2($("#xqbm"));
	...
	{
		ajax : {
			url : CONTEXT_PATH + "/center/creditCheck/queryApplyList.action",
			type : 'post'
		},
		...
		columns : [ {
			...
			render : function(data, type, row) {
				var str = '';
				if ("0" == data) {
					str = '<a href="javascript:;" class="opbtn btn btn-xs green-meadow" onclick="cCreditCheckList.toExamine(\'' + row.id + '\', this)">审核</a>';
				} else {
					str = '<a href="javascript:;" class="opbtn btn btn-xs green-meadow" onclick="cCreditCheckList.toView(\'' + row.id + '\', this)">查看</a>';
				}
				return str;
			}
		} ]
	...
	function conditionReset() {
		resetSearchConditions('#bjbh,#scmc,#xqbm,#examineList_status,#sqsjs,#sqsjz');
		resetDate(start,end);
	}
