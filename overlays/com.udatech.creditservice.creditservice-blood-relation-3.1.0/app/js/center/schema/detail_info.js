var detailInfo = (function(){
	...
	$.getJSON(ctx + "/system/dictionary/listValues.action?groupKey=reportWay", function(result) {
		...
		$('.select2-hidden-accessible.form-search').next('span').css('margin-bottom', '6px');
	...
	if (requiredGroupList == null || requiredGroupList.length == 0) {
		...
		var table = $('#dataReportGrid').DataTable({
			ajax :{
				url: CONTEXT_PATH + '/schema/collection/detailList.action',
				type : 'post',
				data : {
					tableId : logicTableId,
	...
	function queryData(){
		...
		data["reprotDataType"] = $.trim($("#reportWay").val());
	...
	function clearData(){
		...
		resetIEPlaceholder();
		resetSearchConditions('#reprotDataStatus,#departmentDetail,#tableCoulmnName,#showTaskCode,#updateStartDate,#updateEndDate');
		resetSearchConditions('#reportWay');
		resetDate(start, end);
	}
