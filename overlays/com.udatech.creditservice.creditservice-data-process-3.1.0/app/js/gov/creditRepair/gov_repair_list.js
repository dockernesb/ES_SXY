var grl = (function() {
	...
	var table = $('#dataTable').DataTable({
        ajax: {
            url: ctx + "/govRepair/getRepairList.action",
            type: "post"
        },
        ordering: false,
        searching: false,
        ...
        columns: [
            {"data" : "BJBH", "render": function(data, type, full) {
            	var opts = '<a href="javascript:;" onclick="grl.toRepair(\'' + full.ID + '\', \'' + full.DETAIL_ID + '\', this)">' + data + '</a>';
				return opts;
			}},
            {"data" : "QYMC"}, 
            {"data" : "TYSHXYDM"},
            ...
            {"data" : "ID", "render": function(data, type, full) {
            	var opts = '<a href="javascript:;" class="opbtn btn btn-xs green-meadow" onclick="grl.toRepair(\'' + full.ID + '\', \'' + full.DETAIL_ID + '\', this)">查看</a>';
            	if (full.STATUS == "1") {
            		opts += '&nbsp;&nbsp;<a href="javascript:;" class="opbtn btn btn-xs green-meadow" onclick="grl.toRepairAudit(\'' + full.ID + '\', \'' + full.DETAIL_ID + '\', this)">审核</a>';
            	}
				return opts;
			}}
        ],
	...
	function toRepair(id, detailId, _this) {
		...
		var url = ctx + "/govRepair/toRepair.action?id=" + id + "&detailId=" + detailId;
		$("div#childBox").html("");
	...
	function toRepairAudit(id, detailId, _this) {
		...
		var url = ctx + "/govRepair/toRepairAudit.action?id=" + id + "&detailId=" + detailId;
		$("div#childBox").html("");
		$("div#childBox").load(url);
		$("div#childBox").show();
		$("div#parentBox").hide();
