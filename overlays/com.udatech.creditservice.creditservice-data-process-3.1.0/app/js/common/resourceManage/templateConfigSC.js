var templeteConfig = (function () {
    ...
    if (opType == 'add') {
        $('#info_title').html('新建审查报告模板');
    } else if (opType == 'update') {
        $('#info_title').html('编辑审查报告模板');
    }
    ...
    var setting = {
        async: {
            ...
            url: function (treeId, treeNode) {
                var url = ctx + '/creditTemplate/tree.action?templateId=' + templateId + '&opType=' + opType;
    ...
    $(function () {
        if (!isNull(template)) {
            ...
            $('#dataFrom').val(template.dataFrom);
            ...
            if (template.category == 0) {
                ...
                $('input[name="category"][value=1]').iCheck('disable');
            } else if (template.category == 1) {
                $('input[name="category"][value=1]').iCheck('check');
                zTreeObj = $.fn.zTree.init($("#themeTree"), setting);
                $('input[name="category"][value=0]').iCheck('disable');
            }
            addValidator.form();
        }
    });
