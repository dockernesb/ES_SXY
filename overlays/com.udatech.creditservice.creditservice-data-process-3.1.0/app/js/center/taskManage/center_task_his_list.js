var hisTask = function () {
    ...
    function dataDetail(erorData) {
        ...
        if (selectRows.length == 1) {
            ...
            var versionId = row.TABLE_VERSION_ID;//上报数据版本Id

            var url = ctx + '/dp/historyDataReport/dataReport.action?taskCode=' + taskCode + "&erorData=" + erorData + "&parentMenu=center&reportWay=" + reportWay + "&logicTableId=" + id + "&deptId=" + deptId + "&confirmStatus=" + confirmStatus + "&versionId" + versionId;
            $("div#childBox").html("");
            $("div#parentBox").hide();
            $("div#childBox").show();
    ...
    function conditionReset() {
        resetSearchConditions('#tableName,#taskCode,#reportWay,#startDate,#endDate,#status,#conditionDeptId', function () {
            resetDate(start, end);
        });
    }
