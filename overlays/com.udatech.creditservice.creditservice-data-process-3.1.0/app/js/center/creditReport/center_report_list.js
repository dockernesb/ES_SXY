var centerReport = (function () {
    ...
    var table = $('#dataTable').DataTable({
        ...
        columns: [{
            ...
            "data": "xybgbh",
            "render": function (data, type, full) {
                var opts = '<a href="javascript:;" onclick="centerReport.toReportViewPrint(\'' + data + '\', \'' + full.id + '\', this)">' + data + '</a>';
                return opts;
            }
        }, {
            "data": "qymc"
    ...
    function refreshTable(bjbh, jgqc, gszch, zzjgdm, tyshxydm, xybgbh, beginDate, endDate, status) {
        if (table) {
    ...
    function conditionSearch() {
        ...
        refreshTable(bjbh, jgqc, gszch, zzjgdm, tyshxydm, xybgbh, beginDate, endDate, status);
    }

    function conditionReset() {
        if (audit == '1') {// 需要审核时
            resetSearchConditions('#bjbh,#jgqc,#gszch,#zzjgdm,#tyshxydm,#beginDate,#endDate,#xybgbh,#status');
        } else {
            resetSearchConditions('#bjbh,#jgqc,#gszch,#zzjgdm,#tyshxydm,#beginDate,#endDate,#xybgbh');
        }
        resetDate(start, end);
    }
