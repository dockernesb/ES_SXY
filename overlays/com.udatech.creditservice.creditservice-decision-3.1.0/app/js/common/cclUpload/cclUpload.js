...
$.fn.extend({
    ...
    cclUpload : function(opts) {
    	...
    	var url = ctx + "/creditCommon/ajaxFileUpload.action";
        var $self = $(this);
                        ...
                        if (!isNaN(size)) {
                        	if (suffix == "pdf") {
                        		if (size > 20 * 1024 * 1024) {
                        			$.alert("上传pdf文件不能大于20M，请调整文件大小后重新上传！");
                        			$form[0].reset();	//	清空input
                        			return false;
                        		}
                        	} else {
                        		if (size > 10 * 1024 * 1024) {
                        			$.alert("上传图片不能大于10M，请调整图片大小后重新上传！");
                        			$form[0].reset();	//	清空input
                        			return false;
                        		}
                        	}
						...
						if ($target.length == 1) {
							...
							for (var i=0, len=list.length; i<len; i++) {
								...
								if (current.type == "img") {
									...
									html += '<img src="' + ctx + '/creditCommon/viewImg.action?path=' + file.path + '" />';
